...
async def parse_and_index_document_task(
    ...
    logger.info(f"Starting parse and index task {task_id} for doc {doc_id} of request {request_id}")
    ...
    try:
        async with UnitOfWork() as uow:
            ...
            receipt_dir = Path(__file__).parent.parent / "storage" / "requests" / f"REQ-{request_id}" / "documents"
...
async def export_report_task(
    ...
    logger.info(f"Starting report export task {task_id} for report {report_id} (format: {format_str})")
    await TaskStateManager.update_task_status(task_id, "RUNNING")
    
    try:
        from report.services.export_service import ExportService
        receipt = await ExportService.export_report(report_id, format_str, exporter_user)
