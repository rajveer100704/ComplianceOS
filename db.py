...
def add_document(
    ...
    return run_sync(
        PersistenceService.add_document(request_id, filename, text, source_type)
    )
...
def save_claim(request_id: int, run_id: int, document_id: int, result: dict) -> int:
    ...
    return run_sync(
        PersistenceService.save_claim(request_id, run_id, document_id, result)
    )
...
def review_queue() -> list:
    ...
    async def _queue():
        ...
        async with UnitOfWork() as uow:
            stmt = (
                select(ClaimModel, RequestModel.project, RequestModel.regulator)
        ...
        async with UnitOfWork() as uow:
            ...
            if req.status != "needs_review":
                raise TransitionError(
                    f"Cannot transition from '{req.status}' — expected needs_review"
                )
            ...
            stmt_pending = (
                ...
                .where(
                    ...
                    ClaimModel.reviewer_decision == "pending",
                )
            )
    ...
    async def _lock():
        ...
        async with UnitOfWork() as uow:
            ...
            if req.status != "approved":
                raise TransitionError(
                    f"Cannot transition from '{req.status}' — expected approved"
                )
    ...
    async def _submit():
        ...
        async with UnitOfWork() as uow:
            ...
            if req.status != "locked":
                raise TransitionError(
                    f"Cannot transition from '{req.status}' — expected locked"
                )

            req.status = "submitted"
            log_item = AuditLogModel(
                request_id=request_id, stage="submitted", detail="Request submitted"
            )
            await uow.receipts.add(log_item)
            await uow.commit()
    ...
    async def _archive():
        ...
        async with UnitOfWork() as uow:
            ...
            if req.status != "submitted":
                raise TransitionError(
                    f"Cannot transition from '{req.status}' — expected submitted"
                )

            req.status = "archived"
            log_item = AuditLogModel(
                request_id=request_id, stage="archived", detail="Request archived"
            )
            await uow.receipts.add(log_item)
            await uow.commit()
    ...
    async def _stats():
        ...
        async with UnitOfWork() as uow:
            ...
            stmt_locked = (
                select(func.count())
                .select_from(RequestModel)
                .where(RequestModel.status == "locked")
            )
            ...
            stmt_sub = (
                select(func.count())
                .select_from(RequestModel)
                .where(RequestModel.status == "submitted")
            )
            res_sub = await uow.session.execute(stmt_sub)
            submitted_requests = res_sub.scalar() or 0

            stmt_latest_run = (
                select(RunModel.version).order_by(RunModel.id.desc()).limit(1)
            )
...
def report_for_request(request_id: int) -> dict:
    ...
    async def _report():
        ...
        async with UnitOfWork() as uow:
            ...
            receipt = (
                _json.loads(latest_run.receipt)
                if latest_run and latest_run.receipt
                else None
            )
            ...
            open_risks = [
                c for c in latest_claims if c["status"] in ("UNSUPPORTED", "PARTIAL")
            ]
            unresolved_comments = [
                c for c in latest_claims if c["comment"] and not c["resolved"]
            ]
            ...
            for c in open_risks:
                if c["status"] == "UNSUPPORTED":
                    recommendations.append(
                        f"Add supporting evidence or a valid citation for: \"{c['text'][:80]}\""
                    )
                else:
                    recommendations.append(
                        f"Strengthen evidence for partially-supported claim: \"{c['text'][:80]}\""
                    )
            ...
            return {
                ...
                "coverage": coverage,
                "open_risks": [
                    {"claim": c["text"], "status": c["status"], "reason": c["reason"]}
                    for c in open_risks
                ],
                "unresolved_comments": [
                    {"claim": c["text"], "comment": c["comment"]}
