    ...
    async def compare_reports(report_id_a: int, report_id_b: int) -> Dict[str, Any]:
        ...
        async with UnitOfWork() as uow:
            ...
            if not report_a or not report_b:
                raise ValueError(f"One or both reports (IDs: {report_id_a}, {report_id_b}) not found.")

            # Load sections for both
            stmt_sec_a = select(ReportSectionModel).where(ReportSectionModel.report_id == report_id_a, ReportSectionModel.is_deleted == False)
            res_sec_a = await uow.session.execute(stmt_sec_a)
            sections_a = {s.section_type: s for s in res_sec_a.scalars().all()}

            stmt_sec_b = select(ReportSectionModel).where(ReportSectionModel.report_id == report_id_b, ReportSectionModel.is_deleted == False)
            ...
            stmt_find_a = select(ReportFindingModel).where(ReportFindingModel.report_id == report_id_a, ReportFindingModel.is_deleted == False)
            res_find_a = await uow.session.execute(stmt_find_a)
            findings_a = {f.title: f for f in res_find_a.scalars().all()}

            stmt_find_b = select(ReportFindingModel).where(ReportFindingModel.report_id == report_id_b, ReportFindingModel.is_deleted == False)
            ...
            for title, f_b in findings_b.items():
                ...
                if not f_a:
                    ...
                    if f_a.risk_level != f_b.risk_level:
                        modified_fields["risk_level"] = {"from": f_a.risk_level, "to": f_b.risk_level}
                    if f_a.priority != f_b.priority:
                        modified_fields["priority"] = {"from": f_a.priority, "to": f_b.priority}
                    if f_a.recommendation != f_b.recommendation:
                        modified_fields["recommendation"] = {"from": f_a.recommendation, "to": f_b.recommendation}
