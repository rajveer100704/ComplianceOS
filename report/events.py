...
class ReportEventPublisher:
    ...
    @staticmethod
    async def publish_report_generated(report_id: int, request_id: int, version: int, created_by: str) -> None:
    ...
    @staticmethod
    async def publish_report_approved(report_id: int, approved_by: str, timestamp: str) -> None:
    ...
    @staticmethod
    async def publish_report_published(report_id: int, published_by: str, timestamp: str) -> None:
    ...
    @staticmethod
    async def publish_report_archived(report_id: int, archived_by: str, timestamp: str) -> None:
    ...
    @staticmethod
    async def publish_report_exported(report_id: int, format_str: str, exported_by: str, timestamp: str) -> None:
        """Publishes ReportExported domain event."""
        logger.info(f"Publishing ReportExported event for report {report_id} to format {format_str}")
        await EventPublisher.publish_event(
            "ReportExported",
            {
                "report_id": report_id,
