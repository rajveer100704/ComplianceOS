...
class ReportTemplateModel(Base, AuditMixin):
    ...
    sections_config: Mapped[dict] = mapped_column(
        JSON, nullable=False
    )  # JSON list of dicts (title, type, order)
    branding_config: Mapped[dict] = mapped_column(
        JSON, nullable=False
    )  # JSON styling parameters (colors, fonts)
...
class ReportModel(Base, AuditMixin):
    ...
    )  # linked review snapshot version
...
class ReportCitationModel(Base, AuditMixin):
    ...
    finding_id: Mapped[int] = mapped_column(
        ForeignKey("report_findings.id"), nullable=False
    )
    claim_id: Mapped[int] = mapped_column(ForeignKey("claims.id"), nullable=False)
    evidence_id: Mapped[int | None] = mapped_column(
        ForeignKey("pinned_evidences.id"), nullable=True
    )
    comment_id: Mapped[int | None] = mapped_column(
        ForeignKey("claim_comments.id"), nullable=True
    )

    # Relationships
    finding = relationship(
        "ReportFindingModel", back_populates="citations", lazy="raise"
    )
...
class ReportActivityLogModel(Base, AuditMixin):
    ...
    )  # ReportGenerated, ReportApproved, ReportPublished, etc.
