...
class ReviewEventPublisher:
    ...
    @staticmethod
    async def publish_review_assigned(request_id: int, reviewer: str, assigned_by: str) -> None:
    ...
    @staticmethod
    async def publish_evidence_pinned(claim_id: int, chunk_id: str, pinned_by: str, role: str) -> None:
    ...
    @staticmethod
    async def publish_snapshot_created(request_id: int, snapshot_id: int, version: int, creator: str) -> None:
        """Publishes SnapshotCreated domain event."""
        logger.info(f"Publishing SnapshotCreated event for request {request_id} version {version}")
        await EventPublisher.publish_event(
            "SnapshotCreated",
            {
                "request_id": request_id,
    ...
    async def publish_comment_added(comment_id: int, claim_id: int, user: str) -> None:
        ...
        logger.info(f"Publishing CommentAdded event for comment {comment_id} on claim {claim_id}")
        await EventPublisher.publish_event(
            "CommentAdded",
            {
