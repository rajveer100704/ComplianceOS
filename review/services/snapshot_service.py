...
class SnapshotService:
    ...
    async def create_snapshot(request_id: int, creator: str) -> SnapshotReceipt:
        ...
        async with UnitOfWork() as uow:
            ...
            # Publish event
            await ReviewEventPublisher.publish_snapshot_created(request_id, snapshot.id, next_version, creator)
    ...
    @staticmethod
    async def compare_snapshots(request_id: int, version_from: int, version_to: int) -> Dict[str, Any]:
        ...
        async with UnitOfWork() as uow:
            ...
            if not snap_from or not snap_to:
                raise ValueError(f"Snapshot versions ({version_from}, {version_to}) not found for request {request_id}")
            ...
            diff = {
                ...
                "reviewer_changed": payload_from["request"]["assigned_reviewer"] != payload_to["request"]["assigned_reviewer"],
                "claims_modified": [],
                "comments_added": [],
                "evidence_changes": []
            }
            ...
            for cid, c_to in claims_to.items():
                ...
                decision_changed = c_from["reviewer_decision"] != c_to["reviewer_decision"]
                ...
                if decision_changed or resolved_changed or notes_changed:
                    diff["claims_modified"].append({
                        ...
                        "text": c_to["text"],
                        "changes": {
                            "decision": {"from": c_from["reviewer_decision"], "to": c_to["reviewer_decision"]},
                            "resolved": {"from": c_from["resolved"], "to": c_to["resolved"]},
                            "notes": {"from": c_from["review_notes"], "to": c_to["review_notes"]}
                        }
                    })
