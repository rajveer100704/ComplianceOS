...
class EvidenceRepository(BaseRepository[PinnedEvidenceModel]):
    ...
    async def get_by_claim(self, claim_id: int) -> List[PinnedEvidenceModel]:
        ...
        stmt = (
            select(PinnedEvidenceModel)
            .where(PinnedEvidenceModel.claim_id == claim_id, PinnedEvidenceModel.is_deleted == False)
        )
        res = await self.session.execute(stmt)
        return list(res.scalars().all())

    async def get_by_claim_and_chunk(self, claim_id: int, chunk_id: str) -> List[PinnedEvidenceModel]:
