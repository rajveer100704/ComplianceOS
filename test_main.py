...
def test_supported_claim():
    res = client.post("/api/verify", json={
        "claims": ["The vehicle satisfies flight safety analysis requirements for public risk."]
    })
    body = res.json()
...
def test_unsupported_claim():
    res = client.post("/api/verify", json={
        "claims": ["The spacecraft uses lightweight composite panels for thermal protection."]
    })
    body = res.json()
...
def test_request_lifecycle_end_to_end():
    r = client.post("/api/requests", json={
        "project": "Rocket Alpha", "regulator": "FAA", "owner": "Rajveer"
    })
    rid = r.json()["id"]

    client.post(f"/api/requests/{rid}/documents", json={
...
def test_run_without_documents_rejected():
    r = client.post("/api/requests", json={
        "project": "Empty", "regulator": "NRC", "owner": "Rajveer"
    })
    rid = r.json()["id"]
    run = client.post(f"/api/requests/{rid}/run")
...
def test_rerun_creates_new_version_not_overwrite():
    r = client.post("/api/requests", json={"project": "Versioned", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The vehicle satisfies flight safety analysis requirements."
    })
...
def test_pdf_upload_rejects_non_pdf():
    r = client.post("/api/requests", json={"project": "PDF Test", "regulator": "NRC", "owner": "Rajveer"})
    rid = r.json()["id"]
    res = client.post(
...
def test_pdf_upload_accepts_real_pdf():
    ...
    doc = fitz.open()
    page = doc.new_page()
    page.insert_text((72, 72), "The reactor facility safety analysis report describes design bases.")
    pdf_bytes = doc.tobytes()
    doc.close()

    r = client.post("/api/requests", json={"project": "PDF Test 2", "regulator": "NRC", "owner": "Rajveer"})
    rid = r.json()["id"]
    res = client.post(
        f"/api/requests/{rid}/documents/pdf",
...
def test_knowledge_graph_and_coverage():
    r = client.post("/api/requests", json={"project": "Graph Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt",
        "text": "The vehicle satisfies flight safety analysis requirements for public risk. "
...
def test_version_diff_detects_status_change():
    r = client.post("/api/requests", json={"project": "Diff Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The spacecraft uses lightweight composite panels for thermal protection."
    })
    client.post(f"/api/requests/{rid}/run")  # v1: UNSUPPORTED
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "b.txt", "text": "The vehicle satisfies flight safety analysis requirements for public risk."
    })
    client.post(f"/api/requests/{rid}/run")  # v2: adds a SUPPORTED claim too

    diff = client.get(f"/api/requests/{rid}/diff", params={"from_version": 1, "to_version": 2}).json()
    assert any(a["status"] == "SUPPORTED" for a in diff["added"])


def test_dashboard_and_review_queue():
    r = client.post("/api/requests", json={"project": "Dash Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The spacecraft uses lightweight composite panels for thermal protection."
    })
...
def test_comment_and_resolve_claim():
    r = client.post("/api/requests", json={"project": "Comment Test", "regulator": "NRC", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The reactor facility safety analysis report describes design bases and safety limits."
    })
    client.post(f"/api/requests/{rid}/run")
    claim_id = client.get(f"/api/requests/{rid}").json()["claims"][0]["id"]

    res = client.post(f"/api/claims/{claim_id}/comment", json={"comment": "Looks good, double check page ref."})
    assert res.status_code == 200
...
def test_report_builder():
    r = client.post("/api/requests", json={"project": "Report Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt",
        "text": "The vehicle satisfies flight safety analysis requirements for public risk. "
...
def test_submission_workspace_full_lifecycle():
    r = client.post("/api/requests", json={"project": "Submission Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The vehicle satisfies flight safety analysis requirements for public risk."
    })
    client.post(f"/api/requests/{rid}/run")
    ...
    res = client.post(f"/api/requests/{rid}/documents", json={"filename": "b.txt", "text": "late addition"})
    ...
    # can't skip stages
    r2 = client.post("/api/requests", json={"project": "Skip Test", "regulator": "NRC", "owner": "Rajveer"})
    rid2 = r2.json()["id"]
...
def test_reopen_reverts_approval():
    r = client.post("/api/requests", json={"project": "Reopen Test", "regulator": "FAA", "owner": "Rajveer"})
    rid = r.json()["id"]
    client.post(f"/api/requests/{rid}/documents", json={
        "filename": "a.txt", "text": "The vehicle satisfies flight safety analysis requirements for public risk."
    })
    ...
    assert res.status_code == 200
    assert client.get(f"/api/requests/{rid}").json()["request"]["status"] == "needs_review"


def test_multi_sentence_claim_split():
    res = client.post("/api/verify", json={
        "claims": ["The vehicle satisfies flight safety analysis. The reactor has a safety analysis report."]
    })
    body = res.json()
    assert len(body) == 2
...
def test_advanced_pdf_parser_features():
    ...
    doc = fitz.open()
    page = doc.new_page()
    page.insert_text((72, 72), "Header Block: This is a long header to exceed the OCR threshold.")
    page.insert_text((72, 100), "Column A Line 1: Additional safety analysis and details.")
    page.insert_text((200, 100), "Column B Line 1: Additional verification checklists.")
    pdf_bytes = doc.tobytes()
    doc.close()

    # 2. Upload PDF
    r = client.post("/api/requests", json={"project": "Advanced Parser Test", "regulator": "NRC", "owner": "Rajveer"})
    rid = r.json()["id"]
    res = client.post(
        f"/api/requests/{rid}/documents/pdf",
    ...
    meta_path = Path(__file__).parent / "storage" / "requests" / f"REQ-{rid}" / "documents" / "safety_report_advanced.pdf.metadata.json"
    receipt_path = Path(__file__).parent / "storage" / "requests" / f"REQ-{rid}" / "documents" / "safety_report_advanced.pdf.receipt.json"
...
def test_retrieval_architecture_and_pipeline():
    ...
    chunks = chunker.chunk(doc_id=99, text="Paragraph 1\n\n--- Page Break ---\n\nParagraph 2\n\nParagraph 3", doc_metadata={"filename": "test.txt"})
    assert len(chunks) == 3
    ...
    r = client.post("/api/requests", json={"project": "Retrieval Pipeline Test", "regulator": "FAA", "owner": "Alice"})
    rid = r.json()["id"]
    client.post(
        f"/api/requests/{rid}/documents",
        json={"filename": "flight_test.txt", "text": "The vehicle satisfies public risk safety rules."}
    )
    ...
    receipt_path = Path(__file__).parent / "storage" / "requests" / f"REQ-{rid}" / "retrieval.receipt.json"
    assert receipt_path.exists()
    with open(receipt_path, "r", encoding="utf-8") as f:
...
async def test_database_persistence_and_rollback():
    ...
    req_id = await PersistenceService.create_request(project="Integration UOW Test", regulator="FAA", owner="Alice")
    assert req_id is not None

    # 3. Verify Rollback behavior
    try:
        async with UnitOfWork() as uow:
            r = RequestModel(project="Should Be Rolled Back", regulator="NRC", owner="Bob")
    ...
    async with UnitOfWork() as uow:
        stmt = select(RequestModel).where(RequestModel.project == "Should Be Rolled Back")
...
async def test_worker_bootstrap_and_queue_fallback():
    ...
    success, msg = await bootstrap_worker(redis_url="redis://invalid_localhost:12345/0", allow_fallback=True)
...
async def test_outbox_event_dispatching():
    ...
    req_id = await PersistenceService.create_request(project="Outbox Test", regulator="FAA", owner="Alice")
    doc_id = await PersistenceService.add_document(request_id=req_id, filename="test.pdf", text="Some PDF content", source_type="pdf")
    ...
    async with UnitOfWork() as uow:
        ...
        assert len(events) >= 1
        uploaded_event = next((e for e in events if e.payload.get("document_id") == doc_id), None)
    ...
    async with UnitOfWork() as uow:
        ...
        target = next((e for e in processed_events if e.payload.get("document_id") == doc_id), None)
...
async def test_benchmark_runner_execution(tmp_path):
    ...
    for attempt in range(5):
        try:
            ...
            doc_id1 = await PersistenceService.add_document(req_id, "faa_rules.txt", "What safety and risk assessment standards does the FAA enforce for spaceport operations?")
            doc_id2 = await PersistenceService.add_document(req_id, "nrc_rules.txt", "What are the reactor containment safety rules defined by the NRC? safety analysis report guidelines for commercial nuclear power plants.")
            break
        except Exception:
            if attempt == 4:
                raise
    ...
    # 2. Index documents
    indexing_service.index_document(doc_id1, "faa_rules.txt", "What safety and risk assessment standards does the FAA enforce for spaceport operations?")
    indexing_service.index_document(doc_id2, "nrc_rules.txt", "What are the reactor containment safety rules defined by the NRC? safety analysis report guidelines for commercial nuclear power plants.")
    ...
    report_file = workspace_root / "storage" / "reports" / "retrieval_benchmark_report.json"
    summary_file = workspace_root / "storage" / "reports" / "retrieval_benchmark_summary.md"
...
async def test_query_classification():
    ...
    assert QueryClassifier.classify("What are the reactor containment safety rules defined by the NRC OR the FAA AND spaceport operations?") == "hard"
    assert QueryClassifier.classify("Section 4: safety analysis guidelines.") == "medium"
...
async def test_adaptive_top_k_routing():
    ...
    bundle_hard = service.retrieve("What are the reactor containment safety rules defined by the NRC OR the FAA AND spaceport operations?")
...
async def test_review_state_transitions_and_permissions():
    ...
    rid = await PersistenceService.create_request(project="Review State Test", regulator="FAA", owner="Alice")
    ...
    await ReviewService.assign_reviewer(request_id=rid, reviewer="Bob", assigned_by="Alice", role="Reviewer")
    ...
    await ReviewService.transition_status(request_id=rid, new_status="In Review", user="Bob", role="Reviewer")
    ...
    with pytest.raises(ValueError, match="Invalid transition"):
        await ReviewService.transition_status(request_id=rid, new_status="Published", user="Bob", role="Lead Reviewer")
    ...
    with pytest.raises(PermissionError, match="is not authorized"):
        await ReviewService.transition_status(request_id=rid, new_status="Approved", user="Bob", role="Reviewer")

    # Success transition to Approved by Lead Reviewer
    await ReviewService.transition_status(request_id=rid, new_status="Approved", user="Charlie", role="Lead Reviewer")
...
async def test_review_assignment_history_and_immutability():
    ...
    rid = await PersistenceService.create_request(project="Audit Test", regulator="FAA", owner="Alice")
    
    # 1. First assignment
    await ReviewService.assign_reviewer(request_id=rid, reviewer="Bob", assigned_by="Alice", role="Lead Reviewer")
    # 2. Second assignment
    await ReviewService.assign_reviewer(request_id=rid, reviewer="Charlie", assigned_by="Alice", role="Lead Reviewer")
...
async def test_threaded_comments_and_mentions():
    ...
    rid = await PersistenceService.create_request(project="Comments Test", regulator="FAA", owner="Alice")
    doc_id = await PersistenceService.add_document(request_id=rid, filename="notes.txt", text="Claim sentence.", source_type="text")
    ...
    # Add root comment
    root = await CommentService.add_comment(claim_id=claim_id, user="Bob", text="Please double check @Alice.")
    # Add reply comment
    reply = await CommentService.add_comment(claim_id=claim_id, user="Alice", text="All looks good @Bob.", parent_id=root.id)
    ...
    # Verify Mentions
    async with UnitOfWork() as uow:
        res = await uow.session.execute(select(CommentMentionModel).where(CommentMentionModel.comment_id == root.id))
...
async def test_evidence_pinning_and_roles():
    ...
    rid = await PersistenceService.create_request(project="Evidence Test", regulator="FAA", owner="Alice")
    doc_id = await PersistenceService.add_document(request_id=rid, filename="a.txt", text="Source.", source_type="text")
    ...
    await EvidenceService.pin_evidence(claim_id=claim_id, chunk_id="chunk-0", document_id=doc_id, user="Bob", role="PRIMARY")
    # Pin evidence as SUPPORTING
    await EvidenceService.pin_evidence(claim_id=claim_id, chunk_id="chunk-1", document_id=doc_id, user="Bob", role="SUPPORTING")
    ...
    # Unpin evidence
    success = await EvidenceService.unpin_evidence(claim_id=claim_id, chunk_id="chunk-0", user="Bob")
    assert success is True
...
async def test_snapshot_creation_and_semantic_diff():
    ...
    rid = await PersistenceService.create_request(project="Snapshot Test", regulator="FAA", owner="Alice")
    ...
    diff = await SnapshotService.compare_snapshots(request_id=rid, version_from=1, version_to=2)
...
async def test_concurrent_edit_locking_version_check():
    ...
    rid = await PersistenceService.create_request(project="Concurrency Test", regulator="FAA", owner="Alice")
    ...
    async with UnitOfWork() as uow1:
        ...
        if req1.version != db_req.version:
            ...
            with pytest.raises(ValueError, match="Optimistic locking version conflict"):
                raise ValueError("Optimistic locking version conflict: request version mismatch.")
...
async def test_report_generation_and_state_machine():
    ...
    rid = await PersistenceService.create_request(project="Report Machine Test", regulator="FAA", owner="Alice")
    
    async with UnitOfWork() as uow:
        # Create a claim
        from database.models.claim import ClaimModel
        claim = ClaimModel(request_id=rid, text="FAA space launch requirements match safety rules.", status="SUPPORTED", confidence=0.92)
        uow.session.add(claim)
        await uow.commit()
        claim_id = claim.id
    ...
    async with UnitOfWork() as uow:
        ...
        comment = ClaimCommentModel(claim_id=claim_id, user="Alice", text="Matches requirements.", parent_id=None)
        evidence = PinnedEvidenceModel(claim_id=claim_id, chunk_id="1", document_id=1, role="PRIMARY", pinned_by="Alice")
    ...
    template_payload = {
        ...
        "sections_config": {
            "sections": [
                {"title": "Executive Summary", "type": "summary", "order": 1, "default_content": "Summary content"},
                {"title": "Scope", "type": "scope", "order": 2, "default_content": "Scope content"},
                {"title": "Findings", "type": "findings", "order": 3, "default_content": "Findings list"}
            ]
        },
    ...
    async with UnitOfWork() as uow:
        from database.models.report import ReportSectionModel, ReportFindingModel, ReportCitationModel
        from sqlalchemy import select
        
        stmt_sections = select(ReportSectionModel).where(ReportSectionModel.report_id == report.id).order_by(ReportSectionModel.ordering.asc())
        ...
        stmt_findings = select(ReportFindingModel).where(ReportFindingModel.report_id == report.id)
        ...
        # Check citations
        stmt_cit = select(ReportCitationModel).where(ReportCitationModel.finding_id == f.id)
    ...
    await ReportService.transition_status(report_id=report.id, new_status="Generated", user="Alice", role="Reviewer")
    await ReportService.transition_status(report_id=report.id, new_status="Under Review", user="Alice", role="Reviewer")
...
async def test_report_export_validation():
    ...
    rid = await PersistenceService.create_request(project="Export Test", regulator="FAA", owner="Alice")
    
    # Seed claim, snapshot and template
    async with UnitOfWork() as uow:
        from database.models.claim import ClaimModel
        claim = ClaimModel(request_id=rid, text="Export check claim.", status="SUPPORTED", confidence=0.85)
        uow.session.add(claim)
        await uow.commit()
        claim_id = claim.id
    ...
    snap_receipt = await SnapshotService.create_snapshot(request_id=rid, creator="Alice")
    
    async with UnitOfWork() as uow:
        from database.models.report import ReportTemplateModel
        template = ReportTemplateModel(
            name="Export Test Template",
            sections_config={"sections": [{"title": "Scope", "type": "scope", "order": 1}]},
            branding_config={}
        )
        uow.session.add(template)
        await uow.commit()
    ...
    with pytest.raises(ValueError, match="must be Approved or Published to be exported"):
        await ExportService.export_report(report_id=report.id, format_str="html", exporter_user="Alice")

    # Transition to Approved
    await ReportService.transition_status(report_id=report.id, new_status="Generated", user="Alice", role="Reviewer")
    await ReportService.transition_status(report_id=report.id, new_status="Under Review", user="Alice", role="Reviewer")
    await ReportService.transition_status(report_id=report.id, new_status="Approved", user="Bob", role="Lead Reviewer")

    # Perform valid export
    receipt_html = await ExportService.export_report(report_id=report.id, format_str="html", exporter_user="Alice")
    assert receipt_html.format == "html"
    assert os.path.exists(receipt_html.file_path)

    # Perform markdown export
    receipt_md = await ExportService.export_report(report_id=report.id, format_str="markdown", exporter_user="Alice")
    ...
    with pytest.raises(ValueError, match="has no citations to evidence"):
        await ExportService.export_report(report_id=report.id, format_str="json", exporter_user="Alice")
...
async def test_report_semantic_comparison():
    ...
    rid = await PersistenceService.create_request(project="Diff Test", regulator="FAA", owner="Alice")
    
    # Create claims
    async with UnitOfWork() as uow:
        from database.models.claim import ClaimModel
        claim1 = ClaimModel(request_id=rid, text="FAA test claim 1.", status="SUPPORTED", confidence=0.8)
        claim2 = ClaimModel(request_id=rid, text="FAA test claim 2.", status="SUPPORTED", confidence=0.85)
    ...
    assert diff["finding_changes"]["added"][0]["risk_level"] == "Critical"  # Reject mapped to Critical
