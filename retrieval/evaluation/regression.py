...
class RegressionGates:
    ...
    def verify(current_report: dict, baseline_run: dict) -> Tuple[bool, list, list]:
        ...
        curr_m = current_report.get("pipelines", {}).get("hybrid_reranked", {}).get("overall", {})
        base_m = baseline_report.get("pipelines", {}).get("hybrid_reranked", {}).get("overall", {})
        ...
        if not curr_m or not base_m:
            logger.warning("Could not resolve hybrid_reranked metrics for regression checks.")
            return True, [], []
        ...
        if base_recall > 0:
            ...
            if recall_change_pct < -1.0:
                failures.append(f"Recall@10 dropped by {abs(recall_change_pct):.2f}% (Limit: 1.00%)")

        # 3. Check MRR (max drop 2%)
        if base_mrr > 0:
            mrr_change_pct = ((curr_mrr - base_mrr) / base_mrr) * 100
            if mrr_change_pct < -2.0:
                failures.append(f"MRR dropped by {abs(mrr_change_pct):.2f}% (Limit: 2.00%)")
        ...
        if base_lat > 0:
            ...
            if latency_change_pct > 10.0:
                failures.append(f"Latency increased by {latency_change_pct:.2f}% (Limit: 10.00%)")
            elif latency_change_pct > 5.0:
                warnings.append(f"Latency increased by {latency_change_pct:.2f}% (Warning Limit: 5.00%)")
