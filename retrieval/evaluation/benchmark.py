...
class RetrievalBenchmarkRunner:
    ...
    def __init__(self, base_dir: str = None):
        if base_dir is None:
            base_dir = str(
                Path(__file__).parent.parent.parent / "tests" / "retrieval_benchmark"
            )
    ...
    def run_evaluation(self, retrieval_service: RetrievalService) -> dict:
        ...
        def evaluate_config(pipeline_name: str, profile_name: str) -> dict:
            ...
            for q_obj in queries_list:
                ...
                expected_set = {
                    (item["doc_id"], item["index"]) for item in expected_list
                }

                start_time = time.perf_counter()
                bundle = retrieval_service.retrieve(
                    query, limit=10, profile=profile_name
                )
                latency = (time.perf_counter() - start_time) * 1000.0

                retrieved_list = [
                    (chunk.document_id, chunk.metadata.get("paragraph_index", 0))
        ...
        reports = {"pipelines": pipeline_reports, "profiles": profile_reports}
        ...
        with open(
            output_dir / "retrieval_benchmark_report.json", "w", encoding="utf-8"
        ) as f:
            json.dump(reports, f, indent=2)

        self._write_summary_markdown(
            reports, output_dir / "retrieval_benchmark_summary.md"
        )
        return reports
    ...
    def _write_summary_markdown(self, reports: dict, path: Path):
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(
                "| Pipeline | Precision@10 | Recall@10 | MRR | nDCG@10 | Avg Latency (ms) |\n"
            )
            f.write("| --- | --- | --- | --- | --- | --- |\n")
            ...
            f.write(
                "| Profile | Precision@10 | Recall@10 | MRR | nDCG@10 | Avg Latency (ms) |\n"
            )
            f.write("| --- | --- | --- | --- | --- | --- |\n")
            ...
            f.write(
                "| Difficulty | Pipeline | Precision@10 | Recall@10 | MRR | Latency |\n"
            )
            f.write("| --- | --- | --- | --- | --- | --- |\n")
            ...
            if hybrid_mrr >= dense_mrr:
                ...
                f.write(
                    "Reranked hybrid pipeline meets quality thresholds and matches or exceeds dense baseline performance.\n"
                )
            else:
                f.write("### ⚠️ Verification Status: WARNING\n")
                f.write(
                    "Reranked hybrid pipeline MRR degraded compared to dense baseline. Investigation recommended.\n"
                )
