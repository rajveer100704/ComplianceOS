...
from retrieval.base import BaseEmbeddingProvider, BaseVectorStore, BaseRetriever, BaseReranker
...
class RetrievalService:
    ...
    def __init__(self, embedding_provider: BaseEmbeddingProvider, vector_store: BaseVectorStore, planner, reranker: BaseReranker = None, cache = None):
        self.raw_embedding_provider = embedding_provider
        if cache:
            self.embedding_provider = CachedEmbeddingProviderProxy(embedding_provider, cache)
        else:
        ...
        self.pipeline = RetrievalPipeline(self.dense_retriever, self.bm25_retriever, self.rrf, reranker)

    def retrieve(self, query: str, limit: int = 5, filters: dict = None, profile: str = None) -> EvidenceBundle:
        # 1. Validation
        ...
        profiles = ret_conf.get("profiles", {
            "fast": {"dense_top_k": 10, "lexical_top_k": 10, "rrf_k": 40, "rerank_limit": 3},
            "balanced": {"dense_top_k": 20, "lexical_top_k": 20, "rrf_k": 60, "rerank_limit": 5},
            "quality": {"dense_top_k": 50, "lexical_top_k": 50, "rrf_k": 60, "rerank_limit": 10}
        })
        ...
        if adaptive_enabled:
            if difficulty == "easy":
                prof_params["dense_top_k"] = max(10, int(prof_params["dense_top_k"] * 0.5))
                prof_params["lexical_top_k"] = max(10, int(prof_params["lexical_top_k"] * 0.5))
                prof_params["rerank_limit"] = max(2, int(prof_params["rerank_limit"] * 0.5))
        ...
        with measure_time() as t:
            # Execute search
            candidates = self.pipeline.run(state, plan, limit, filters=filters, profile_params=prof_params)
        ...
        if isinstance(self.embedding_provider, CachedEmbeddingProviderProxy):
            total = self.embedding_provider.cache_hits + self.embedding_provider.cache_misses
            if total > 0:
